...
namespace Logica.Datos
{
    ...
    {        
        ...
        public static void ModificarUsuario(Persona usuario)
        {
            try
            {
                ...
                using (StreamWriter sw = new StreamWriter(filePath))
                {
                    foreach (string line in lines)
                    {
                        ...
                        if (campos.Length == 5 && campos[2] == usuario.Usuario)
                        {
                            // Encontramos el usuario a modificar, escribimos la nueva información
                            sw.WriteLine("{0},{1},{2},{3},{4}", usuario.Id, usuario.Nombre, usuario.Usuario, usuario.Contrasenia, usuario.Role);
                        }
                        else
                        {
                            // No es el usuario a modificar, simplemente copiamos la línea original
                            sw.WriteLine(line);
                        }
                    }
                }
            ...
            {
                throw new Exception("Error al modificar el usuario en el archivo: " + error.Message);
            }
        }
    }
}
